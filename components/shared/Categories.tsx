...
const getCategories = async ({ locale }: { locale: string }) => {
  let res = await axios.get(`${apiUrl}/categories?type=service&lang=${locale}`)
  return res.data
}
...
const Categories = () => {
  ...
  return (
    <div className="px-4 mt-4">
      ...
      <div className="grid grid-cols-4 gap-3 mt-2 max-xl:grid-cols-3">
      ...
      >
        <div className="grid grid-cols-4 sm:grid-cols-6 gap-4">
          ...
          {!isLoadingServices && servicesData?.data?.length === 0 && (
            <p className="text-center text-gray-500 col-span-2">{t("no_services_found")}</p>
          )}
        </div>
      </BottomSheet>
    </div>
